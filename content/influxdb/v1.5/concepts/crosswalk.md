...
- You can [drop or delete a series](/influxdb/v1.5/query_language/database_management/#drop-series-from-the-index-with-drop-series),
  but not individual points based on field values. As a workaround, you can search
  for the field value, retrieve the time, then [DELETE based on the `time` field](/influxdb/v1.5/query_language/database_management/#delete-series-with-delete).

- You can't update or rename tags yet - see GitHub issue [#4157](https://github.com/influxdata/influxdb/issues/4157)
  for more information. To modify the tag of a series of points, find the points
  with the offending tag value, change the value to the desired one, write the
  points back, then drop the series with the old tag value.

- You can't delete tags by tag key (as opposed to value) - see GitHub issue
  [#8604](https://github.com/influxdata/influxdb/issues/8604).
